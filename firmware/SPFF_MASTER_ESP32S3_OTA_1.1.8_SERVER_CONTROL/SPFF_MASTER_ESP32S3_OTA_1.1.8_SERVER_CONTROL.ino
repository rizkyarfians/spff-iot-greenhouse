...
/*
  =============================================================================
  S2PFS MASTER SPFF FIRMWARE 1.1.8 SERVER CONTROL OTA
  ...
  - 16 program ON/OFF mingguan lokal tetap tersimpan di NVS untuk rollback,
    tetapi tidak menulis relay selama backend menjadi pemilik aktuator.
  - Jadwal produksi dievaluasi Orange Pi dan dieksekusi ESP32 melalui command
    set_pump yang sama dengan kontrol manual; relay boot tetap OFF.
...
static constexpr char SPFF_FIRMWARE_VERSION[] = "1.1.8";

// Rollout minimal production:
// - true  : Orange Pi menjadi satu-satunya pemilik jadwal dan mengirim set_pump.
//           Timer lokal LCD/NVS tidak boleh menimpa command remote.
// - false : perilaku legacy; timer mingguan lokal kembali mengendalikan CH3.
//
// Jangan ubah menjadi false selama MQTT Worker memakai
// SCHEDULE_EXECUTION_MODE=server.
static constexpr bool SPFF_BACKEND_OWNS_ACTUATORS = true;
...
// Batas frame command serial. Payload command normal jauh lebih kecil dari ini;
// parser akan discard seluruh frame sampai newline jika batas terlewati.
static constexpr size_t SPFF_RX_LINE_MAX = 2048;
...
const char* timerModeText(
  ...
  if (SPFF_BACKEND_OWNS_ACTUATORS) {
    return "BACKEND";
  }
...
bool sendSpffActuatorAck(
  ...
  // Jangan membuat timestamp palsu. Command tidak dieksekusi bila RTC invalid.
...
void processSpffCommandLine(const String &line) {
  ...
  if (kind != "command") {
    // Serial channel juga membawa ACK/persisted message lain; abaikan yang bukan command.
    return;
  }
  ...
  if (nowEpoch == 0UL) {
    // Tanpa waktu UTC valid firmware tidak dapat membuktikan command belum expired.
    return;
  }
...
void appSetup() {
  ...
  spffRxLine.reserve(SPFF_RX_LINE_MAX);
