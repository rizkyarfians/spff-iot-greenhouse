...
/*
  =============================================================================
  S2PFS MASTER SPFF FIRMWARE 1.3.2 SENSOR HEALTH OTA
...
static constexpr char SPFF_FIRMWARE_VERSION[] = "1.3.2";
...
enum SpffSystemState : uint8_t {
  ...
  SPFF_SYSTEM_DEGRADED = 2,
  SPFF_SYSTEM_SENSOR_FAULT = 3
};
...
void updateSystemState(bool anySensorValid, bool allSensorsValid);
void captureTelemetrySnapshot();
bool telemetrySnapshotHasAnyValidData();
uint8_t telemetrySnapshotValidFieldCount();
uint8_t telemetrySnapshotHealthyFieldCount();
void spffAppendTelemetrySensorHealth(String &json);
...
void updateSystemState(
  ...
  } else if (anySensorValid) {
    nextState =
      SPFF_SYSTEM_DEGRADED;
...
uint8_t telemetrySnapshotHealthyFieldCount() {
  uint8_t count = telemetrySnapshotValidFieldCount();

  // Nilai last-known SHT20 tetap boleh dikirim untuk diagnosis, tetapi health
  // kedua parameternya mengikuti hasil pembacaan SHT20 pada cycle sekarang.
  if (!shtTelemetryValid()) {
    if (cachedFloatValid(telemetrySnapshot.airTemp) && count > 0U) {
      count--;
    }
    if (cachedFloatValid(telemetrySnapshot.airHumidity) && count > 0U) {
      count--;
    }
  }
...
void spffAppendSensorHealthEntry(
  String &json,
  const char *key,
  bool valid,
  const char *reason,
  ...
  spffAppendKey(json, key, first);
  json += "{\"valid\":";
  json += valid ? "true" : "false";

  if (!valid && reason != nullptr && reason[0] != '\0') {
    json += ",\"reason\":\"";
    json += reason;
    json += '"';
  }
...
void spffAppendTelemetrySensorHealth(String &json) {
  bool first = true;
  const bool shtCurrentValid = shtTelemetryValid();

  spffAppendSensorHealthEntry(json, "liquid_ph", cachedFloatValid(telemetrySnapshot.liquidPh), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "liquid_ec_us_cm", cachedFloatValid(telemetrySnapshot.liquidEcUsCm), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "liquid_temp", cachedFloatValid(telemetrySnapshot.liquidTemp), "stale_or_missing", first);

  const bool airTempCached = cachedFloatValid(telemetrySnapshot.airTemp);
  const bool airHumidityCached = cachedFloatValid(telemetrySnapshot.airHumidity);
  spffAppendSensorHealthEntry(json, "air_temp", airTempCached && shtCurrentValid, airTempCached ? "read_error" : "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "air_humidity", airHumidityCached && shtCurrentValid, airHumidityCached ? "read_error" : "stale_or_missing", first);

  spffAppendSensorHealthEntry(json, "soil_1_moisture", cachedFloatValid(telemetrySnapshot.soil1Moisture), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "soil_1_temp", cachedFloatValid(telemetrySnapshot.soil1Temp), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "soil_1_ec_us_cm", cachedUIntValid(telemetrySnapshot.soil1EcUsCm), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "soil_1_ph", cachedFloatValid(telemetrySnapshot.soil1Ph), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "soil_1_n", cachedUIntValid(telemetrySnapshot.soil1N), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "soil_1_p", cachedUIntValid(telemetrySnapshot.soil1P), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "soil_1_k", cachedUIntValid(telemetrySnapshot.soil1K), "stale_or_missing", first);

  spffAppendSensorHealthEntry(json, "soil_2_moisture", cachedFloatValid(telemetrySnapshot.soil2Moisture), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "soil_2_temp", cachedFloatValid(telemetrySnapshot.soil2Temp), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "soil_2_ec_us_cm", cachedUIntValid(telemetrySnapshot.soil2EcUsCm), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "soil_2_ph", cachedFloatValid(telemetrySnapshot.soil2Ph), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "soil_2_n", cachedUIntValid(telemetrySnapshot.soil2N), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "soil_2_p", cachedUIntValid(telemetrySnapshot.soil2P), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "soil_2_k", cachedUIntValid(telemetrySnapshot.soil2K), "stale_or_missing", first);

  spffAppendSensorHealthEntry(json, "tank_water_distance_cm", cachedFloatValid(telemetrySnapshot.tankWaterDistanceCm), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "tank_fert_distance_cm", cachedFloatValid(telemetrySnapshot.tankFertDistanceCm), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "flow_water_lpm", cachedFloatValid(telemetrySnapshot.flowWaterLpm), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "flow_fert_lpm", cachedFloatValid(telemetrySnapshot.flowFertLpm), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "flow_water_total_l", cachedFloatValid(telemetrySnapshot.flowWaterTotalL), "stale_or_missing", first);
  spffAppendSensorHealthEntry(json, "flow_fert_total_l", cachedFloatValid(telemetrySnapshot.flowFertTotalL), "stale_or_missing", first);
}
...
void sendTelemetryIfValid() {
  ...
  const uint8_t healthySnapshotFields =
    telemetrySnapshotHealthyFieldCount();

  const bool anySensorValid =
    healthySnapshotFields > 0U;

  const bool allSensorsValid =
    healthySnapshotFields ==
...
bool sendSpffTelemetry() {
  ...
  spffAppendTelemetrySensorHealth(json);
...
static bool spffParseWaterAutomaticConfig(
  ...
  bool &tankLevelIgnored,
  const char *&reason
) {
  tankLevelIgnored = false;
  bool enabled = false;
  String sensorKey;
  float low = 0;
  ...
  tankLevelIgnored = !minTankNull;

  if (
    (sensorKey != "soil_1_moisture" && sensorKey != "soil_2_moisture") ||
    (!minTankNull && (minTank < 0 || minTank > 100)) ||
...
void processSpffAutomaticControlSyncLine(
  ...
  bool waterTankLevelIgnored = false;
  bool fertilizerTankLevelIgnored = false;
  if (
  ...
  spffSendAutomaticControlAck(
    ...
    waterTankLevelIgnored || fertilizerTankLevelIgnored
      ? "tank_level_ignored"
      : ""
  );
}
