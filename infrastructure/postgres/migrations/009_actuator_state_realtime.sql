...
CREATE INDEX IF NOT EXISTS actuator_state_site_recorded_idx
  ON spff.actuator_state_events
  (site_id, recorded_at DESC, actuator_state_id DESC);
...
BEGIN
  event_type := CASE TG_TABLE_NAME
    ...
    WHEN 'actuator_state_events' THEN 'actuator_state.updated'
    ELSE NULL
  END;
...
DROP TRIGGER IF EXISTS actuator_state_realtime_notify_trg
  ON spff.actuator_state_events;

CREATE TRIGGER actuator_state_realtime_notify_trg
AFTER INSERT ON spff.actuator_state_events
FOR EACH ROW
EXECUTE FUNCTION spff.notify_realtime_event();

COMMENT ON FUNCTION spff.notify_realtime_event() IS
  'Publishes compact post-commit telemetry, device status, and actuator state signals for the local SSE API.';
